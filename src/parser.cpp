...
void parse_sphere(char *str, matrix& edges){
	double r; std::vector<double> pc = std::vector<double>(4, 0); int step = 40;
	char **args = parse_args(str);
	for(int i = 0; i < 3; i++){
		pc[i] = atof(args[i]);
	}
...
void parse_torus(char *str, matrix& edges){
	double r; double R; std::vector<double> pc = std::vector<double>(4, 0); int step = 40;
	char **args = parse_args(str);
	for(int i = 0; i < 3; i++){
		pc[i] = atof(args[i]);
	}
...
void parse_file ( const char * filename, 
	...
	while ( (f.rdstate() & std::ifstream::eofbit ) == 0 ){
		if(strncmp(str, "line", 250) == 0){
			...
			f.getline(str, 250);
			parse_sphere(str, edges);
		}

		else if(strncmp(str, "torus", 250) == 0){
			f.getline(str, 250);
			parse_torus(str, edges);
		}
